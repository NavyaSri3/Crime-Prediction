...
def main():
    ...
    while again:
        p = float(input('Percent of data to train on: '))
        ran = input('Shuffle data?(y/n) ')
        if ran == 'y' or ran == 'Y':
            ran = True
        else:
            ran = False
        ...
        while new_lam:
            ...
            xopt = fmin_bfgs(costFunction, Theta, 
                             fprime=gradient, args=(X,Y,lam), maxiter = 100
                             )
            #xopt = minimize(costFunction, Theta, args=(X,Y,lam), method=None, jac=gradient, options={'maxiter':50})
            ...
            new_lam = input('Different lambda?(y/n) ')
            if new_lam == 'y' or new_lam == 'Y':
                new_lam = True
            else:
                new_lam = False

        ans = input('Create submission file?(y/n) ')
        if ans == 'y' or ans == 'Y':
            ...
            sub_file = input('Enter submission file name: ')
            f = open(sub_file, 'w')
            header ='Id'
            for c in crimes:
                header += ',' + c
